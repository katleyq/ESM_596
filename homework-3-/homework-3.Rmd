...
Given a feature vector \( \mathbf{x} = (x_1, x_2, \dots, x_n) \) and a target class label \( y \in \{0, 1\} \), a linear classifier aims to find a **decision boundary** in the form of a hyperplane that separates the classes.

The decision rule for a linear classifier is:

\[
y = \begin{cases} 
1, & \text{if } \mathbf{\beta} \cdot \mathbf{x} + \beta_0 > 0 \\
0, & \text{if } \mathbf{\beta} \cdot \mathbf{x} + \beta_0 \leq 0 
\end{cases}
\]

where:

- \( \mathbf{\beta} = (\beta_1, \beta_2, \dots, \beta_n) \) is the vector of coefficients, representing the importance of each feature in classification.
- \( \beta_0 \) is the intercept, allowing the decision boundary to be shifted from the origin.
- \( \mathbf{\beta} \cdot \mathbf{x} \) represents the **dot product** between the coefficient vector and the feature vector.

a. In a logistic regression, assume the decision rule for classifying \( \mathbf{x} \) is to assign class 1 if \( P(y = 1 | \mathbf{x}) > p^* \) and class 0 otherwise. Show that this decision rule leads to a linear classifier.
   

b. Given a two-class Linear Discriminant Analysis (LDA) model with classes \( y \in \{0, 1\} \) and feature vector \( \mathbf{x} \), assume that the data in each class follows a multivariate normal distribution with class-specific means \( \boldsymbol{\mu}_0 \) and \( \boldsymbol{\mu}_1 \) and a common covariance matrix \( \Sigma \). Show that LDA a linear classifier. *Hint*: Use the log of the posterior ratio and show that the decision boundary can be expressed as a linear function of \( \mathbf{x} \).
